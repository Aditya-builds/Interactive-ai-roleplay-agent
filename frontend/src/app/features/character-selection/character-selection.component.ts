...
export class CharacterSelectionComponent implements OnInit {
  ...
  worldDisplayName(worldId: string): string {
    const names: Record<string, string> = {
      shattered_world: 'Varellia',
      fantasy: 'Fantasy'
    };
    return names[worldId] ?? this.formatLabel(worldId);
  }

  storyTagList(story: Story): string[] {
    const tags = [this.worldDisplayName(story.worldId)];
